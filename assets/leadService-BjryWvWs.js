import{o as c}from"./index-CMn9DqN2.js";
...
const getLeads = async (params = {}) => {
  try {
    ...
    if (res.data && res.data.success) {
      const raw = Array.isArray(res.data.data) ? res.data.data : (Array.isArray(res.data.leads) ? res.data.leads : []);
      const items = raw.map(item => ({
        ...item,
        _id: item._id || item.id,
        name: item.name || item.customerName || "Lead",
        company: item.company || "Direct Client",
        value: item.value !== undefined ? item.value : (item.amount || 0),
        priority: item.priority || "high",
        score: item.score || 85,
        classification: item.classification || "hot"
      }));
      return { success: true, data: items, total: res.data.pagination ? res.data.pagination.total : items.length };
    }
  } catch {}
...
const createLead = async (leadData) => {
  ...
  const custName = leadData.customerName || leadData.name || "New Lead";
  const contactNum = leadData.phone || leadData.contact || "";
  ...
  try {
    ...
    const res = await c.post("/leads", payload);
    if (res.data && res.data.success) {
      const item = res.data.lead || res.data.data || res.data;
      const normalized = {
        ...item,
        _id: item._id || item.id,
        name: item.name || item.customerName || custName,
        company: item.company || leadData.company || "Direct Client",
        value: numVal,
        priority: item.priority || leadData.priority || "high",
        status: item.status || leadData.status || "new"
      };
      const leads = getStoredLeads();
      saveStoredLeads([normalized, ...leads]);
      return { success: true, data: normalized, lead: normalized };
    }
  } catch {}

import { c, w as se, r as t, j as e, a as te, x as ae, f as le, X as re, n as D, p as oe, M as de } from "./index-CMn9DqN2.js";
    ...
    try {
      ...
      if (parsed && typeof parsed === "object") {
        return {
          ...
          email: parsed.email || "admin@empirecrm.io",
          role: parsed.role || "Super Admin",
          phone: parsed.phone || "+91 98765 43210"
        };
      }
    } catch (e) {}
    return { name: "Admin User", email: "admin@empirecrm.io", role: "Super Admin", phone: "+91 98765 43210" };
  });
      ...
      e.jsxs("div", {
        ...
        children: [
          ...
          e.jsxs("div", {
            ...
            children: [
              e.jsxs("div", { children: [e.jsx("label", { className: "block text-slate-500 mb-1", children: "Full Name" }), e.jsx("input", { type: "text", readOnly: true, value: (user && (user.name || user.fullName)) || "Admin User", className: "w-full px-4 py-2.5 rounded-xl bg-slate-200/60 dark:bg-slate-700/60 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 outline-none" })] }),
              e.jsxs("div", { children: [e.jsx("label", { className: "block text-slate-500 mb-1", children: "Email Address" }), e.jsx("input", { type: "email", readOnly: true, value: (user && user.email) || "admin@empirecrm.io", className: "w-full px-4 py-2.5 rounded-xl bg-slate-200/60 dark:bg-slate-700/60 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 outline-none" })] })
            ]
          })
        ]
      }),
          ...
          e.jsxs("div", {
            ...
            children: [
              ...
              e.jsxs("div", {
                className: "flex items-center gap-2",
                children: [
                  e.jsx("input", { type: "text", value: newLeave, onChange: (ev) => setNewLeave(ev.target.value), placeholder: "e.g. Sabbatical", className: "flex-1 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 outline-none" }),
...
function SettingsPageWrapper() {
  const [activeSettingsTab, setActiveSettingsTab] = t.useState(() => {
    const p = typeof window !== "undefined" ? window.location.pathname : "";
    if (p.includes("/settings/web")) return "web";
    if (p.includes("/settings/lead-trash") || p.includes("/settings/trash")) return "trash";
    if (p.includes("/settings/lead")) return "lead";
    if (p.includes("/settings/hrms")) return "hrms";
    if (p.includes("/settings/integrations")) return "integrations";
    if (p.includes("/settings/attributes")) return "attributes";
    if (p.includes("/settings/templates")) return "templates";
    if (p.includes("/settings/automation")) return "automation";
    return "account";
  });
  ...
  const handleTabClick = (tabKey) => {
    ...
    let prefix = "";
    if (p.startsWith("/crm/business")) prefix = "/crm/business";
    else if (p.startsWith("/crmbusiness")) prefix = "/crmbusiness";
    else {
      const crmMatch = p.match(/^(\/crm\/[^\/]+)/);
      if (crmMatch) prefix = crmMatch[1];
    }
    const routeMap = {
      account: prefix + "/settings/account-security",
      web: prefix + "/settings/web",
      lead: prefix + "/settings/lead",
      hrms: prefix + "/settings/hrms",
      integrations: prefix + "/settings/integrations",
      trash: prefix + "/settings/lead-trash",
      attributes: prefix + "/settings/attributes",
      templates: prefix + "/settings/templates",
      automation: prefix + "/settings/automation-rules"
    };
    const targetUrl = routeMap[tabKey] || (prefix + "/settings");

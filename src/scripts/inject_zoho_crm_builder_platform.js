...
console.log('=== INJECTING ZOHO-STYLE CRM BUILDER PLATFORM & DYNAMIC WORKSPACES ===');

// 1. Sidebar Navigation - Add "Create Your Own CRM" under Tools & Settings
const sidebarTarget = `{name:"CRM Builder",path:"/crm-builder",icon:Gc}`;
const sidebarFullTarget = `{title:"TOOLS & SETTINGS",items:[`;
const sidebarReplacement = `{title:"TOOLS & SETTINGS",items:[{name:"Create Your Own CRM",path:"/crm-builder",icon:Gc},{name:"My CRMs",path:"/my-crms",icon:xD},`;

if (code.includes(sidebarFullTarget) && !code.includes('Create Your Own CRM')) {
  code = code.replace(sidebarFullTarget, sidebarReplacement);
  console.log('[SUCCESS] Added "Create Your Own CRM" & "My CRMs" to Sidebar TOOLS & SETTINGS!');
} else if (code.includes(sidebarTarget)) {
  code = code.replace(sidebarTarget, `{name:"Create Your Own CRM",path:"/crm-builder",icon:Gc},{name:"My CRMs",path:"/my-crms",icon:xD}`);
  console.log('[SUCCESS] Updated Sidebar menu item to "Create Your Own CRM"!');
}

// 2. Full Multi-Step CRM Builder Component (<CRMBuilderComponent />)
const zohoCRMBuilderJs = `
function CRMBuilderComponent() {
      ...
      return saved ? JSON.parse(saved) : [
        {
          ...
          customModule: {
            ...
            fields: [
              { fieldLabel: "Property Name", fieldName: "property_name", fieldType: "Text", isRequired: true },
              { fieldLabel: "Property Type", fieldName: "property_type", fieldType: "Dropdown", isRequired: true },
              { fieldLabel: "Listing Price", fieldName: "price", fieldType: "Currency", isRequired: true },
              { fieldLabel: "Location", fieldName: "location", fieldType: "Text", isRequired: false },
              { fieldLabel: "Available Date", fieldName: "available_date", fieldType: "Date", isRequired: false }
            ]
          },
          pipeline: [
  ...
  const [activeTab, setActiveTab] = rt.useState("builder");
  ...
  const handleCreateCRM = async () => {
    ...
    setLoading(true);
    setStatusMsg("Provisioning CRM workspace in Supabase backend...");

    const crmId = "crm-" + Date.now();
    const slug = crmData.slug || crmData.name.toLowerCase().replace(/[^a-z0-9]/g, "-");

    try {
      const token = localStorage.getItem("accessToken");
      const userEmail = "admin." + Date.now() + "@" + (crmData.companyName || "crm").toLowerCase().replace(/[^a-z0-9]/g, "") + ".com";
      ...
      wizardStep === 1 ? d.jsxs("div", {
        ...
        children: [
          ...
          d.jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-6 text-xs font-semibold", children: [
            ...
            d.jsxs("div", { children: [d.jsx("label", { className: "block font-bold mb-1", children: "Workspace Slug / URL" }), d.jsx("input", { type: "text", value: crmData.slug, onChange: e => setCrmData({ ...crmData, slug: e.target.value }), placeholder: "real-estate-crm", className: "w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 outline-none font-mono" })] })
          ]}),
...
// 3. My CRMs Catalog Component (<MyCRMsComponent />)
const myCRMsJs = `
      ...
      d.jsxs("div", {
        ...
        children: [
          d.jsxs("div", { children: [
            ...
            d.jsx("p", { className: "text-sm text-slate-500 dark:text-slate-400 mt-1 font-medium", children: "All multi-tenant CRM workspaces configured and created under your account." })
          ]}),
        ...
        children: workspaces.map(ws => d.jsxs("div", {
          ...
          children: [
            ...
            d.jsxs("div", { className: "flex items-center space-x-2 pt-3 border-t border-slate-100 dark:border-slate-800", children: [
              d.jsx("button", { onClick: () => nav("/my-crms/" + ws.id), className: "flex-1 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold text-xs shadow-md transition cursor-pointer text-center", children: "Open CRM Workspace →" }),
...
// 4. Dynamic CRM Workspace Layout Component (<DynamicWorkspaceComponent />)
const dynamicWorkspaceJs = `
function DynamicWorkspaceComponent() {
  ...
  rt.useEffect(() => {
    try {
      ...
      const found = list.find(w => w.id === idOrSlug || w.slug === idOrSlug) || list[0];
      setWs(found);
    } catch { setWs(null); }
  }, []);

  if (!ws) {
    return d.jsx("div", { className: "p-12 text-center text-slate-400 font-bold", children: "Loading workspace..." });
  }
    ...
    children: [
      d.jsxs("div", {
        ...
        children: [
          d.jsxs("div", { children: [
            d.jsxs("div", { className: "flex items-center space-x-3", children: [
              d.jsx("span", { className: "p-2 rounded-2xl bg-indigo-600 text-white shadow-lg text-xl", children: "🏢" }),
              d.jsxs("div", { children: [
                d.jsx("h1", { className: "text-2xl sm:text-3xl font-black text-slate-900 dark:text-white", children: ws.name }),
                d.jsx("p", { className: "text-xs text-slate-500 dark:text-slate-400 font-semibold", children: ws.companyName + " • " + ws.industry + " Workspace" })
              ]})
            ]})
          ]}),
          d.jsxs("div", { className: "flex items-center space-x-3", children: [
            d.jsx("button", { onClick: () => nav("/my-crms"), className: "px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 font-bold text-xs hover:bg-slate-100 dark:hover:bg-slate-800 transition cursor-pointer", children: "← All CRMs" }),
            d.jsx("button", { onClick: () => nav("/dashboard"), className: "px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold text-xs shadow-md cursor-pointer", children: "Launch Main Dashboard" })
          ]})
        ]
      }),

      d.jsxs("div", { className: "space-y-4", children: [
        d.jsx("h2", { className: "text-base font-black text-slate-900 dark:text-white", children: "📦 Active Workspace Modules" }),
        d.jsx("div", {
          className: "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4",
          children: (ws.modules || ["leads", "contacts", "companies", "deals", "tasks", "reports"]).concat(ws.customModule ? [ws.customModule.name] : []).map(modName => {
            const isCustom = ws.customModule && ws.customModule.name === modName;
            const routePath = isCustom ? "/records/" + (ws.customModule.slug || modName.toUpperCase()) : "/" + modName.toLowerCase();
            return d.jsxs("div", {
              key: modName,
              onClick: () => nav(routePath),
              className: "p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:border-indigo-500 transition cursor-pointer flex flex-col justify-between space-y-3",
              children: [
                d.jsxs("div", { className: "flex items-center justify-between", children: [
                  d.jsx("h3", { className: "font-black text-sm text-slate-900 dark:text-white capitalize", children: modName }),
                  d.jsx("span", { className: "px-2 py-0.5 rounded-full text-[10px] font-black uppercase " + (isCustom ? "bg-purple-500/10 text-purple-500" : "bg-indigo-500/10 text-indigo-500"), children: isCustom ? "Custom Entity" : "Standard" })
                ]}),
                d.jsx("span", { className: "text-xs font-bold text-indigo-600 dark:text-indigo-400 flex items-center space-x-1", children: "Open Module →" })
              ]
            });
          })
        })
...
// Insert components into index-CMn9DqNx.js
let builderIdx = code.indexOf('function CRMBuilderComponent()');
if (builderIdx !== -1) {
  const fdStart = code.indexOf('function FD()', builderIdx);
  if (fdStart !== -1) {
    code = code.substring(0, builderIdx) + zohoCRMBuilderJs + '\n' + myCRMsJs + '\n' + dynamicWorkspaceJs + '\n' + code.substring(fdStart);
    console.log('[SUCCESS] Replaced CRMBuilderComponent and added MyCRMsComponent + DynamicWorkspaceComponent!');
  }
} else {
  const fdStart = code.indexOf('function FD()');
  if (fdStart !== -1) {
    code = code.substring(0, fdStart) + zohoCRMBuilderJs + '\n' + myCRMsJs + '\n' + dynamicWorkspaceJs + '\n' + code.substring(fdStart);
    console.log('[SUCCESS] Injected Zoho-Style CRM Builder platform components!');
  }
}

// Mount routes in FD()
const platformRoutes = `d.jsx(oe,{path:"/my-crms",element:d.jsx(ve,{children:d.jsx(ge,{children:d.jsx(MyCRMsComponent,{})})})}),d.jsx(oe,{path:"/my-crms/:crmId",element:d.jsx(ve,{children:d.jsx(ge,{children:d.jsx(DynamicWorkspaceComponent,{})})})}),d.jsx(oe,{path:"/crm/:crmSlug",element:d.jsx(ve,{children:d.jsx(ge,{children:d.jsx(DynamicWorkspaceComponent,{})})})}),`;

if (!code.includes('path:"/my-crms"')) {
  const fdStart = code.indexOf('function FD()');
  if (fdStart !== -1) {
    const loginIdx = code.indexOf('path:"/login"', fdStart);
    if (loginIdx !== -1) {
      code = code.substring(0, loginIdx) + platformRoutes + code.substring(loginIdx);
      console.log('[SUCCESS] Mounted /my-crms and /crm/:crmSlug routes in FD()!');
    }
  }
}

fs.writeFileSync(bundlePath, code, 'utf8');
console.log('Platform injection complete.');

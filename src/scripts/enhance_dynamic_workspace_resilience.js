...
console.log('=== ENHANCING DYNAMIC WORKSPACE SLUG RESOLUTION & BACKEND PERSISTENCE ===');

const enhancedDynamicWorkspaceJs = `
  ...
  const loadWorkspace = async () => {
    ...
    try {
      ...
      if (!found && list.length > 0) {
        found = list.find(w => w.slug.includes("real-estate") || w.slug.includes("sales")) || list[0];
      }

      if (!found) {
        // Attempt backend fetch from Supabase v1 modules API
        try {
          ...
          if (modData && modData.modules) {
            found = {
              ...
              companyName: "Empire Workspace",
              industry: "Real Estate",
              description: "Custom generated CRM workspace powered by Supabase backend.",
              modules: modData.modules.map(m => m.moduleKey.toLowerCase()),
              customModule: {
                name: "Properties",
                slug: "properties",
                description: "Real Estate Property Catalog",
                fields: [
...
const dwStart = code.indexOf('function DynamicWorkspaceComponent()');
if (dwStart !== -1) {
  const myCrmsStart = code.indexOf('function MyCRMsComponent()', dwStart);
  if (myCrmsStart !== -1) {
    code = code.substring(0, dwStart) + enhancedDynamicWorkspaceJs + '\n' + code.substring(myCrmsStart);
    fs.writeFileSync(bundlePath, code, 'utf8');
    console.log('[SUCCESS] Replaced DynamicWorkspaceComponent with enhanced backend-resilient implementation!');
  }
}
